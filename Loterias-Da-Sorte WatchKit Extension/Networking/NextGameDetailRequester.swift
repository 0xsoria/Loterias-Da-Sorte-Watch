...
struct GameDetailModel {
    let gameData: LotteryNetworkingWorker
    let headers: [String]
}

final class GameDetailRequester: GameRequesterProtocol {
    ...
    private func checkLottery(lottery: LotteryGamesNoSpace, data: Data) {
        ...
        case .diadesorte:
            self.resultDecoded(data: data) { [weak self] (result: DiaDeSorteConcourseData) in
                ...
                self?.returnData = GameDetailModel(gameData: result.diadesorteWorkerDataToLotteryWorker(), headers: GameDetailData().federal)
